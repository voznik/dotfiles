...
function lust.it(name, fn)
    ...
    if err then print(indent(lust.level + 1) .. red .. tostring(err) .. normal) end
...
local paths = {
    ...
    have = {
        test = function(v, x)
            if type(v) ~= "table" then error("expected " .. tostring(v) .. " to be a table") end
    ...
    setmetatable(assertion, {
        ...
        __call = function(t, ...)
            if paths[t.action].test then
                ...
                if not res then error(err or "unknown failure", 2) end
            end
        end,
    })
