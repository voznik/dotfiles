...
function UpdateGooseConfig(provider, model)
    local safe_provider = SanitizeAlphanumeric(provider)
    local safe_model = SanitizeAlphanumeric(model)
    local sed_cmd = string.format("sed -i 's/^GOOSE_PROVIDER:.*/GOOSE_PROVIDER: %s/' %s", safe_provider, CONFIG_FILE)
    os.execute(sed_cmd)
    sed_cmd = string.format("sed -i 's/^GOOSE_MODEL:.*/GOOSE_MODEL: %s/' %s", safe_model, CONFIG_FILE)
    os.execute(sed_cmd)
end
