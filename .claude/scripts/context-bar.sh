...
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
	...
	# Build bar by batching consecutive same-color characters
	accent_chars="" half_chars="" empty_chars=""
	...
	bar=""
	[[ -n "$accent_chars" ]] && bar+="${C_ACCENT}${accent_chars}"
	[[ -n "$half_chars" ]] && bar+="${C_ACCENT}${half_chars}"
	[[ -n "$empty_chars" ]] && bar+="${C_BAR_EMPTY}${empty_chars}"
	bar+="${C_RESET}"
...
# Build output: Model | Dir | Branch (uncommitted) | Context | Last Message
